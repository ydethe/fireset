...
class CollectionPartSync(CollectionPartCache, CollectionPartHistory, CollectionBase):
    def sync(self, old_token: str = "") -> Tuple[str, Iterable[str]]:
        # The sync token has the form http://.org/ns/sync/TOKEN_NAME
        ...
        if old_token:
            ...
            if not old_token.startswith("http://.org/ns/sync/"):
                raise ValueError("Malformed token: %r" % old_token)
            old_token_name = old_token[len("http://.org/ns/sync/") :]
        ...
        token = "http://.org/ns/sync/%s" % token_name
        if token_name == old_token_name:
            # Nothing changed
            return token, ()
        token_folder = os.path.join(
            self._filesystem_path, ".Radicale.cache", "sync-token"
        )
