...
MIMETYPE_TAGS: Mapping[str, str] = {
    value: key for key, value in xmlutils.MIMETYPES.items()
}
...
def prepare(
    ...
    write_whole_collection: Optional[bool] = None,
) -> Tuple[
            ...
            if write_whole_collection and tag == "VCALENDAR":
                ...
                for _, components in vobject_components_by_uid:
                    ...
                    item = radicale_item.Item(
                        collection_path=collection_path, vobject_item=vobject_collection
                    )
                    item.prepare()
                    items.append(item)
                ...
                for vobject_item in vobject_items:
                    item = radicale_item.Item(
                        collection_path=collection_path, vobject_item=vobject_item
                    )
                    item.prepare()
                    items.append(item)
...
class ApplicationPartPut(ApplicationBase):
    def do_PUT(
        ...
        try:
            ...
            logger.warning(
                "Bad PUT request on %r (read_request_body): %s", path, e, exc_info=True
            )
        ...
        content_type = environ.get("CONTENT_TYPE", "").split(";", maxsplit=1)[0]
        try:
            ...
            if self._log_bad_put_request_content:
                ...
                logger.debug(
                    "Bad PUT request content: suppressed by config/option [auth] bad_put_request_content"
                )
            return httputils.BAD_REQUEST
        (
            prepared_items,
        ...
        with self._storage.acquire_lock("w", user):
            ...
            parent_item = next(iter(self._storage.discover(access.parent_path)), None)
            if not isinstance(parent_item, storage.BaseCollection):
                return httputils.CONFLICT

            write_whole_collection = (
                isinstance(item, storage.BaseCollection) or not parent_item.tag
            )
            ...
            if (
                tag != prepared_tag
                or prepared_write_whole_collection != write_whole_collection
            ):
                (
                    prepared_items,
                    prepared_tag,
            ...
            if prepared_exc_info:
                logger.warning(
                    "Bad PUT request on %r (prepare): %s",
                    path,
                    prepared_exc_info[1],
            ...
            if write_whole_collection:
                try:
                    ...
                    logger.warning(
                        "Bad PUT request on %r (create_collection): %s",
                        path,
                        e,
                        exc_info=True,
                    )
                ...
                if (
                    item
                    and item.uid != prepared_item.uid
                    or not item
                    and parent_item.has_uid(prepared_item.uid)
                ):
                    return self._webdav_error_response(
                        client.CONFLICT,
                        "%s:no-uid-conflict" % ("C" if tag == "VCALENDAR" else "CR"),
                    )
