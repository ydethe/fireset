...
def xml_report(
    ...
    if root.tag in (
        ...
        logger.warning(
            "Unsupported REPORT method %r on %r requested",
            xmlutils.make_human_tag(root.tag),
            path,
        )
        return client.MULTI_STATUS, multistatus
    if (
        root.tag == xmlutils.make_clark("C:calendar-multiget")
        and collection.tag != "VCALENDAR"
        or root.tag == xmlutils.make_clark("CR:addressbook-multiget")
        and collection.tag != "VADDRESSBOOK"
        or root.tag == xmlutils.make_clark("D:sync-collection")
        and collection.tag not in ("VADDRESSBOOK", "VCALENDAR")
    ):
        logger.warning(
            "Invalid REPORT method %r on %r requested",
            xmlutils.make_human_tag(root.tag),
            path,
        )
    ...
    if root.tag in (
        ...
        for href_element in root.findall(xmlutils.make_clark("D:href")):
            ...
            if (href_path + "/").startswith(base_prefix + "/"):
                ...
                logger.warning(
                    "Skipping invalid path %r in REPORT request on " "%r",
                    href_path,
                    path,
                )
        ...
        try:
            ...
            # Invalid sync token
            logger.warning(
                "Client provided invalid sync token %r: %s",
                old_sync_token,
                e,
                exc_info=True,
            )
            # client.CONFLICT doesn't work with some clients (e.g. InfCloud)
            return (client.FORBIDDEN, xmlutils.webdav_error("D:valid-sync-token"))
        hreferences = (
            pathutils.unstrip_path(posixpath.join(collection.path, n)) for n in names
        )
        ...
        hreferences = (path,)
    filters = root.findall(xmlutils.make_clark("C:filter")) + root.findall(
        xmlutils.make_clark("CR:filter")
    )

    # Retrieve everything required for finishing the request.
    retrieved_items = list(
        retrieve_items(base_prefix, path, collection, hreferences, filters, multistatus)
    )
    ...
    while retrieved_items:
        ...
        if filters and not filters_matched:
            try:
                if not all(
                    test_filter(collection_tag, item, filter_) for filter_ in filters
                ):
                    continue
            except ValueError as e:
                raise ValueError(
                    "Failed to filter item %r from %r: %s"
                    % (item.href, collection.path, e)
                ) from e
            ...
            if prop.tag == xmlutils.make_clark("D:getetag"):
                ...
                if expand is not None:
                    ...
                    if (start is None) or (end is None):
                        return client.FORBIDDEN, xmlutils.webdav_error("C:expand")

                    start = datetime.datetime.strptime(start, "%Y%m%dT%H%M%SZ").replace(
                        tzinfo=datetime.timezone.utc
                    )
                    end = datetime.datetime.strptime(end, "%Y%m%dT%H%M%SZ").replace(
                        tzinfo=datetime.timezone.utc
                    )

                    expanded_element = _expand(element, copy.copy(item), start, end)
    ...
    if rruleset:
        ...
        for recurrence_dt in recurrences:
            ...
            vevent.recurrence_id = ContentLine(
                name="RECURRENCE-ID",
                value=recurrence_utc.strftime(dt_format),
                params={},
            )
...
def _make_vobject_expanded_item(
    ...
    if dt_end is not None:
        if type(vevent.dtend.value) is datetime.date:
            end_utc = datetime.datetime.fromordinal(dt_end.value.toordinal()).replace(
                tzinfo=datetime.timezone.utc
            )
        else:
...
def retrieve_items(
    ...
    collection: storage.BaseCollection,
    hreferences: Iterable[str],
    filters: Sequence[ET.Element],
    multistatus: ET.Element,
) -> Iterator[Tuple[radicale_item.Item, bool]]:
...
def test_filter(
    collection_tag: str, item: radicale_item.Item, filter_: ET.Element
) -> bool:
    """Match an item against a filter."""
    if collection_tag == "VCALENDAR" and filter_.tag != xmlutils.make_clark(
        "C:%s" % filter_
    ):
        if len(filter_) == 0:
            return True
    ...
    if collection_tag == "VADDRESSBOOK" and filter_.tag != xmlutils.make_clark(
        "CR:%s" % filter_
    ):
        ...
        if test == "anyof":
            return any(
                radicale_filter.prop_match(item.vobject_item, f, "CR") for f in filter_
            )
        if test == "allof":
            return all(
                radicale_filter.prop_match(item.vobject_item, f, "CR") for f in filter_
    ...
    raise ValueError("Unsupported filter %r for %r" % (filter_.tag, collection_tag))
