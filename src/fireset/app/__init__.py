...
Can be used with an external WSGI server (see ``.application()``) or
the built-in server (see ``.server`` module).
...
class Application(
    ...
    ApplicationPartHead,
    ApplicationPartGet,
    ApplicationPartMkcalendar,
    ApplicationPartMkcol,
    ApplicationPartMove,
    ApplicationPartOptions,
    ApplicationPartPropfind,
    ApplicationPartProppatch,
    ApplicationPartPost,
    ApplicationPartPut,
    ApplicationPartReport,
    ApplicationBase,
):
    """WSGI application."""
    ...
    def __init__(self, configuration: config.Configuration) -> None:
        ...
        self._bad_put_request_content = configuration.get(
            "logging", "bad_put_request_content"
        )
        self._request_header_on_debug = configuration.get(
        ...
        self._permit_delete_collection = configuration.get(
    ...
    def _scrub_headers(self, environ: types.WSGIEnviron) -> types.WSGIEnviron:
        ...
        if self._mask_passwords and headers.get("HTTP_AUTHORIZATION", "").startswith(
            "Basic"
        ):
    ...
    def __call__(
        self, environ: types.WSGIEnviron, start_response: types.WSGIStartResponse
    ) -> Iterable[bytes]:
        with log.register_stream(environ["wsgi.errors"]):
            try:
                ...
                logger.error(
                    "An exception occurred during %s request on %r: " "%s",
                ...
                status, raw_headers, raw_answer = httputils.INTERNAL_SERVER_ERROR
    ...
    def _handle_request(self, environ: types.WSGIEnviron) -> _IntermediateResponse:
        ...
        def response(
            ...
            status_text = "%d %s" % (status, client.responses.get(status, "Unknown"))
            logger.info(
        ...
        if self._request_header_on_debug:
            ...
            logger.debug(
                "Request header: suppressed by config/option [auth] request_header_on_debug"
            )
        ...
        base_prefix_src = (
            "HTTP_X_SCRIPT_NAME" if "HTTP_X_SCRIPT_NAME" in environ else "SCRIPT_NAME"
        )
        base_prefix = environ.get(base_prefix_src, "")
        if base_prefix and base_prefix[0] != "/":
            logger.error(
                "Base prefix (from %s) must start with '/': %r",
                base_prefix_src,
                base_prefix,
            )
        ...
        if base_prefix.endswith("/"):
            logger.warning(
                "Base prefix (from %s) must not end with '/': %r",
                base_prefix_src,
                base_prefix,
            )
        ...
        if path.rstrip("/").endswith("/.well-known/caldav") or path.rstrip(
            "/"
        ).endswith("/.well-known/carddav"):
            return response(
                *httputils.redirect(base_prefix + "/", client.MOVED_PERMANENTLY)
            )
        ...
        if user:
            ...
            with self._storage.acquire_lock("r", user):
                principal = next(
                    iter(self._storage.discover(principal_path, depth="1")), None
                )
            if not principal:
                if "W" in self._rights.authorization(user, principal_path):
                    with self._storage.acquire_lock("w", user):
                        try:
                            ...
                            if new_coll:
                                jsn_coll = self.configuration.get(
                                    "storage", "predefined_collections"
                                )
                                for (name_coll, props) in jsn_coll.items():
                                    try:
                                        self._storage.create_collection(
                                            principal_path + name_coll, props=props
                                        )
                                    except ValueError as e:
                                        logger.warning(
                                            "Failed to create predefined collection %r: %s",
                                            name_coll,
                                            e,
                                        )
                        except ValueError as e:
                    ...
                    logger.warning(
                        "Access to principal path %r denied by " "rights backend",
                        principal_path,
                    )
        ...
        if self._internal_server:
            ...
            if content_length:
                if (
                    ...
                    and content_length > self._max_content_length
                ):
        ...
        if (
            (status, headers, answer) == httputils.NOT_ALLOWED
            and not user
            and not external_login
        ):
            # Unknown or unauthorized user
            logger.debug("Asking client for authentication")
            status = client.UNAUTHORIZED
            headers = dict(headers)
            headers.update({"WWW-Authenticate": 'Basic realm="%s"' % self._auth_realm})
