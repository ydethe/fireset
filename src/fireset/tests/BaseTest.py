...
class BaseTest:
        ...
        **kwargs
        ...
        if login:
            environ["HTTP_AUTHORIZATION"] = (
                "Basic " + base64.b64encode(login.encode(encoding)).decode()
            )
    ...
    def parse_responses(text: str) -> RESPONSES:
        ...
        path_responses: Dict[str, Union[int, Dict[str, Tuple[int, ET.Element]]]] = {}
        for response in xml.findall(xmlutils.make_clark("D:response")):
            ...
            for propstat in response.findall(xmlutils.make_clark("D:propstat")):
    ...
    def get(self, path: str, check: Optional[int] = 200, **kwargs) -> Tuple[int, str]:
    ...
    def post(
        ...
        **kwargs
        ...
        status, _, answer = self.request("POST", path, data, check=check, **kwargs)
        return status, answer

    def put(
        self, path: str, data: str, check: Optional[int] = 201, **kwargs
    ) -> Tuple[int, str]:
        status, _, answer = self.request("PUT", path, data, check=check, **kwargs)
        return status, answer
    ...
    def propfind(
        ...
        path: str,
        data: Optional[str] = None,
        check: Optional[int] = 207,
        **kwargs
    ) -> Tuple[int, RESPONSES]:
        status, _, answer = self.request("PROPFIND", path, data, check=check, **kwargs)
    ...
    def proppatch(
        ...
        **kwargs
        ...
        status, _, answer = self.request("PROPPATCH", path, data, check=check, **kwargs)
        if status < 200 or 300 <= status:
            return status, {}
        assert answer is not None
        responses = self.parse_responses(answer)
    ...
    def report(
        self, path: str, data: str, check: Optional[int] = 207, **kwargs
    ) -> Tuple[int, RESPONSES]:
        status, _, answer = self.request("REPORT", path, data, check=check, **kwargs)
        if status < 200 or 300 <= status:
            return status, {}
        assert answer is not None
    ...
    def delete(
        self, path: str, check: Optional[int] = 200, **kwargs
    ) -> Tuple[int, RESPONSES]:
        assert "data" not in kwargs
    ...
    def mkcalendar(
        ...
        path: str,
        data: Optional[str] = None,
        check: Optional[int] = 201,
        **kwargs
    ) -> Tuple[int, str]:
        status, _, answer = self.request(
            "MKCALENDAR", path, data, check=check, **kwargs
        )
        return status, answer
    ...
    def create_addressbook(
        self, path: str, check: Optional[int] = 201, **kwargs
    ) -> int:
        assert "data" not in kwargs
        return self.mkcol(
            path,
