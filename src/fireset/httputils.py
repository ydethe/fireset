...
def decode_request(
    configuration: "config.Configuration", environ: types.WSGIEnviron, text: bytes
) -> str:
...
def read_raw_request_body(
    configuration: "config.Configuration", environ: types.WSGIEnviron
) -> bytes:
...
def read_request_body(
    ...
    if configuration.get("logging", "request_content_on_debug"):
        ...
        logger.debug(
            "Request content: suppressed by config/option [auth] request_content_on_debug"
        )
    return content
...
def _serve_traversable(
    traversable: _TRAVERSABLE_LIKE_TYPE,
    base_prefix: str,
    path: str,
    path_prefix: str,
    index_file: str,
    ...
    if path != path_prefix and not path.startswith(path_prefix):
        raise ValueError(
            "path must start with path_prefix: %r --> %r" % (path_prefix, path)
        )
    ...
    for part in parts:
        if not pathutils.is_safe_filesystem_path_component(part):
            ...
            return NOT_FOUND
        if not traversable.is_dir() or all(
            part != entry.name for entry in traversable.iterdir()
        ):
            return NOT_FOUND
    ...
    if isinstance(traversable, pathlib.Path):
        headers["Last-Modified"] = time.strftime(
            "%a, %d %b %Y %H:%M:%S GMT", time.gmtime(traversable.stat().st_mtime)
        )
...
def serve_resource(
    ...
    if sys.version_info < (3, 9):
        traversable = pathlib.Path(pkg_resources.resource_filename(package, resource))
    else:
        traversable = resources.files(package).joinpath(resource)
    return _serve_traversable(
        traversable,
        base_prefix,
        path,
        path_prefix,
